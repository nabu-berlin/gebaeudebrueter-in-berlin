(function(){
      ...
      var FAVICON_CANDIDATES = ['docs/images/vogel.png', 'images/vogel.png'];
      ...
      function ensureBirdFavicon(){
        if(!document || !document.head){ return; }
        var candidates = FAVICON_CANDIDATES.slice();
        (function tryNext(){
          if(!candidates.length){ return; }
          var candidate = candidates.shift();
          var probe = new Image();
          probe.onload = function(){ setFaviconHref(candidate); };
          probe.onerror = function(){ tryNext(); };
          try{ probe.src = candidate; }catch(e){ tryNext(); }
        })();
      }
      ...
      (function initMobileRedesign(){
        ...
        var startupSplashShown = false;
        var startupSplashTimer = null;
        var startupSplashOpenedAt = 0;
        var STARTUP_SPLASH_MIN_MS = 700;
        var STARTUP_SPLASH_FALLBACK_MS = 7000;
        ...
        function clearStartupSplashTimer(){
          if(startupSplashTimer){
            try{ clearTimeout(startupSplashTimer); }catch(e){}
            startupSplashTimer = null;
          }
        }
        function restoreCoverCloseButton(){
          if(!mobileRefs || !mobileRefs.coverClose){ return; }
          mobileRefs.coverClose.style.removeProperty('display');
          mobileRefs.coverClose.removeAttribute('aria-hidden');
          mobileRefs.coverClose.disabled = false;
        }
        function hideStartupSplash(force){
          if(!startupSplashShown){ return; }
          var elapsed = Date.now() - startupSplashOpenedAt;
          if(!force && elapsed < STARTUP_SPLASH_MIN_MS){
            clearStartupSplashTimer();
            startupSplashTimer = setTimeout(function(){ hideStartupSplash(true); }, STARTUP_SPLASH_MIN_MS - elapsed);
            return;
          }
          clearStartupSplashTimer();
          startupSplashShown = false;
          restoreCoverCloseButton();
          closeCoverModal(false);
        }
        function showStartupSplashWhileLoading(){
          if(!isMobile || !mobileRefs || !mobileRefs.coverModal || startupSplashShown){ return; }
          startupSplashShown = true;
          startupSplashOpenedAt = Date.now();
          if(mobileRefs.coverClose){
            mobileRefs.coverClose.style.display = 'none';
            mobileRefs.coverClose.setAttribute('aria-hidden', 'true');
            mobileRefs.coverClose.disabled = true;
          }
          openCoverModal();

          var closeOnReady = function(){ hideStartupSplash(false); };
          if(document.readyState === 'complete'){
            setTimeout(closeOnReady, 0);
          } else {
            addCleanup(window, 'load', closeOnReady, { once: true });
          }
          if(MS.map && typeof MS.map.whenReady === 'function'){
            try{ MS.map.whenReady(closeOnReady); }catch(e){}
          }
          clearStartupSplashTimer();
          startupSplashTimer = setTimeout(function(){ hideStartupSplash(true); }, STARTUP_SPLASH_FALLBACK_MS);
        }
