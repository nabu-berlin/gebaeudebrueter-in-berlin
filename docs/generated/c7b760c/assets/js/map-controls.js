(function(){
      ...
      function parseMetaFromIconHtml(html){
        ...
        var buildingType = ''; var buildingName = '';
        if(el){
      ...
      function rebuildCluster(selectedSpecies, selectedStatus, selectedBuildingTypes){
        if(!MS.ready){ return; }
        ...
        var buildingAll = selectedBuildingTypes.length === ALL_BUILDING_TYPES.length;
        var toAdd = [];
        for(var i=0;i<MS.markers.length;i++){
          var m = MS.markers[i];
          var sp = m._ms.species || [];
          var st = m._ms.statuses || [];
          var bt = String(m._ms.buildingType || '').trim().toLowerCase();
          // Group semantics:
          ...
          var buildingAllows = buildingAll ? true : (selectedBuildingTypes.length ? selectedBuildingTypes.indexOf(bt) !== -1 : false);
      ...
      function buildFilters(){
        ...
        var hasBuildingUi = !!(bRow && bAccordion);
      ...
      function applyFilters(){
        ...
        var selectedBuildingTypes = buildingBoxes.length
          ? Array.from(document.querySelectorAll('.ms-filter-building:checked')).map(function(el){ return el.value; })
          : ALL_BUILDING_TYPES.slice();
        if(!MS.ready){ setTimeout(function(){ rebuildCluster(selectedSpecies, selectedStatus, selectedBuildingTypes); }, 150); }
        else { rebuildCluster(selectedSpecies, selectedStatus, selectedBuildingTypes); }
      ...
      function updateFilterFabIndicator(){
        ...
        var buildingChecked = buildingBoxes.length ? Array.prototype.filter.call(buildingBoxes, function(el){ return el.checked; }).length : ALL_BUILDING_TYPES.length;
        var buildingTotal = buildingBoxes.length ? buildingBoxes.length : ALL_BUILDING_TYPES.length;
        var allSelected = (speciesChecked === speciesBoxes.length) && (statusChecked === statusBoxes.length) && (buildingChecked === buildingTotal);
      ...
      function wireFilters(){
        ...
        function resetFiltersToAll(){
          ...
          rebuildCluster(Object.keys(SPECIES_COLORS_JS), Object.keys(STATUS_INFO_JS), Object.keys(BUILDING_TYPE_INFO_JS));
          updateFilterFabIndicator();
      ...
      (function initMobileRedesign(){
        ...
        function bindMobileEvents(){
          ...
          addCleanup(document, 'change', function(ev){
            ...
            if(ev.target.classList && (ev.target.classList.contains('ms-filter-species') || ev.target.classList.contains('ms-filter-status') || ev.target.classList.contains('ms-filter-building'))){
              updateFilterFabIndicator();
              return;
            }
            if(/^ms-(species|status|building)-all/.test(ev.target.id || '')){ updateFilterFabIndicator(); }
          });
      ...
      buildFilters();
      wireFilters();
      setTimeout(function(){ var selectedSpecies = Object.keys(SPECIES_COLORS_JS); var selectedStatus = Object.keys(STATUS_INFO_JS); var selectedBuildingTypes = Object.keys(BUILDING_TYPE_INFO_JS); rebuildCluster(selectedSpecies, selectedStatus, selectedBuildingTypes); }, 250);
    })();
